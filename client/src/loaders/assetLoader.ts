...
export async function loadJson(url: string): Promise<object> {
	return new Promise<object>((resolve, reject) =>
		getFileLoader().load(url, (fileContents: string) => resolve(JSON.parse(fileContents)), undefined, reject)
	);
}

const __currentlyLoadingTextureResolvers = new Map<string, Array<(texture: Texture) => void>>();
export async function loadTexture(url: string, flipY?: boolean): Promise<Texture> {
	...
	if (__currentlyLoadingTextureResolvers.has(url)) {
		...
		promise = new Promise<Texture>((resolve, reject) => {
			...
			const onLoad = (texture: Texture) => {
				...
				__currentlyLoadingTextureResolvers.get(url)!.forEach(resolve => resolve(texture));
