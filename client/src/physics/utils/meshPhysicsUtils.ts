...
const __colliderLoopPositionsCache: Map<string, Vec2[][]> = new Map();
...
export async function getArchitectMeshAndFixtures(
	...
	const key = `${meshName}:${colliderName}`;
	if (!__colliderLoopPositionsCache.has(key)) {
		...
		if (vertsAttribute instanceof BufferAttribute && indicesAttribute instanceof BufferAttribute) {
			...
			__colliderLoopPositionsCache.set(
				key,
				loops.map(loop => {
					return loop.map(i => {
						const i3 = i * 3;
	...
	for (const positions of __colliderLoopPositionsCache.get(key)!) {
