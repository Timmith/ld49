...
function updateVals(timeStamp: number, valsNow: number[], valsSrc: number[], callbacks: CB[][]) {
	...
	constructor(gamePad: Gamepad) {
		...
		setInterval(() => {
			...
			if (lastTimestamp !== gamePad.timestamp) {
				...
				updateVals(gamePad.timestamp, this.axesNow, gamePad.axes as number[], this.axesCallbacks);
