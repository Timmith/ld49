...
export class Explosion extends NoduleSphere {
	readonly explodingState = new AnimatedBool(v => this.updateExplosion(v), false, 15000, Easing.Exponential.Out);
	private _explosionSize: number;
	private mat: MeshBasicMaterial;
	constructor(private _radius: number, nodules: number = 30, depth: number = 4, size: number = 0) {
