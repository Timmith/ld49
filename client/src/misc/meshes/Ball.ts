...
export class Ball extends NoduleSphere {
	...
	readonly explodingState = new AnimatedBool(v => this.updateExplosion(v), false, 0, Easing.Exponential.Out, 1000);
	readonly validTargets: this[] = [];
	collider: Mesh;
	private _explosionSize: number;
	constructor(private _radius: number, useCustomCollider = false, nodules: number = 30, depth: number = 4) {
