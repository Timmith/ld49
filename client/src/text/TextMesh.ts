...
import { listenToProperty, stopListeningToProperty } from "~/utils/propertyListeners";
...
export class Gradient {
	...
	constructor(options: GradientOptions) {
		Object.keys(options).forEach((key: Extract<keyof GradientOptions, string>) => {
...
export default class TextMesh extends Mesh {
	...
	set color(value: Color | Gradient | string | number) {
		...
		const colorBufferAttribute = (this.geometry as BufferGeometry).getAttribute("color") as BufferAttribute;
	...
	onFontFaceChange = (newFontFace: FontFace, oldFontFace: FontFace) => {
		if (oldFontFace) {
			stopListeningToProperty(oldFontFace, "msdfTexture", this.onFontTextureUpdate);
	...
	onBeforeRender = (
		...
		if (this.settings.screenSpace) {
			...
			__mat.multiplyMatrices(camera.matrixWorldInverse, this.matrixWorld).premultiply(camera.projectionMatrix); //.multiply(camera.projectionMatrix)
		...
		if (this.dirty) {
			...
			if (this.newTexture) {
				if (this.optimizeRenderOrder) {
					this.renderOrder = this.renderOrder || 100 + getFontFaceSubOrder(this.newTexture);
				}
				(this.material as RawShaderMaterial).uniforms.msdf.value = this.newTexture;
	...
	onAdd() {
		if (this.livePropObject && this.livePropName) {
			listenToProperty(this.livePropObject, this.livePropName, this.updateText, true);
		}
	}
	...
	onRemove() {
		...
		if (this.livePropObject && this.livePropName) {
			stopListeningToProperty(this.livePropObject, this.livePropName, this.updateText);
		}
		if (this._fontFace) {
			stopListeningToProperty(this._fontFace, "msdfTexture", this.onFontTextureUpdate);
	...
	private regenerateGeometry() {
		...
		this.geometry = tryCreateTextGeometry(this._text, this.settings, this._globalColor);
...
const createTextGeometry = (
	...
	const geometry = createGeometry({
		...
		width: settings.width !== undefined ? (settings.width / settings.size) * font.info.size : undefined,
		lineHeight: settings.lineHeight !== undefined ? settings.lineHeight * font.info.size : undefined,
	...
	segments.forEach(segment => {
		const { topLeft, topRight, bottomLeft, bottomRight } = toVertexColors(segment.color);
...
const toColor = (value: Color | string | number) => (value instanceof Color ? value : new Color(value));

const toGradient = (value: Color | Gradient | string | number) =>
	value instanceof Gradient ? value : new Gradient({ top: value, bottom: value });
