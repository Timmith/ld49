...
export function get2DPositionOnPlane(camera: Camera, cameraWorldPos: Vector3, x: number, y: number, plane: Plane) {
...
export function hitTestPlaneAtPixel(x: number, y: number, plane: Plane, camera: Camera) {
	...
	if (__ray.intersectPlane(plane, __targetVec)) {
		return __targetVec;
	} else {
		return null;
	}
...
export function copyXY(source: XY, destination: XY) {
	destination.x = source.x;
	destination.y = source.y;
}

export function zeroXY(destination: XY) {
	destination.x = 0;
	destination.y = 0;
}
...
export function distanceTo(initialPosition: Vec2, endPosition: Vec2) {
	return Math.sqrt(distanceSquaredTo(initialPosition, endPosition));
}
