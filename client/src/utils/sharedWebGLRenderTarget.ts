...
export const prototypePlaneGeometryUvAttribute = new PlaneBufferGeometry(1, 1, 1, 1).attributes.uv as BufferAttribute;
...
export class RenderTargetSubRegion {
	...
	render(renderer: WebGLRenderer, scene: Scene, camera: Camera, width: number, height: number, forceClear?: boolean) {
	...
	updateUvRegion(index: number, tilesX: number, tilesY: number) {
		const min = this.uvRegion.min;
		const max = this.uvRegion.max;
		min.x = (index % this.sharedRenderTarget.cols) / this.sharedRenderTarget.cols;
		min.y = ~~(index / this.sharedRenderTarget.cols) / this.sharedRenderTarget.rows;
		max.copy(min);
