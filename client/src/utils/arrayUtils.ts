...
export const removeFromArray = <T>(arr: T[], value: T, strict: boolean = false) => {
	const index = arr.indexOf(value);
	if (index !== -1) {
		arr.splice(index, 1);
	} else if (strict) {
...
export const replaceManyInArray = <T>(arr: Array<T | undefined>, value: T, replacement?: T) => {
...
export const pushToArrayMap = <T, T2>(map: Map<T, T2[]>, key: T, value: T2, oneCopyMax: boolean = false) => {
	if (!map.has(key)) {
		map.set(key, []);
	}
...
export const cleanRemoveFromArrayMap = <T, T2>(map: Map<T, T2[]>, key: T, value: T2) => {
	if (!map.has(key)) {
		return;
	}
	const arr = map.get(key) as T2[];
