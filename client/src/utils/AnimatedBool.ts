...
import { NumberEaser } from "~/misc/animation/RawTweener";
...
export class AnimatedBool {
	...
	set value(val: boolean) {
		...
		const duration =
			(desiredVal > this._animatedValue ? this._durationMS : this._durationMSOut) *
			Math.abs(this._animatedValue - desiredVal);
