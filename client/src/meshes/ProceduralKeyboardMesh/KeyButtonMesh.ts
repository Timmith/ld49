...
export default class KeyButtonMesh extends Mesh {
	constructor(width: number, height: number, depth: number, label: string = "", chamfer: number = 0.005) {
		super(getCachedChamferedBoxGeometry(width, height, depth, chamfer), materialLibrary.keyboardPlasticKey);
		if (label !== "") {
