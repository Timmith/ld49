...
class KeyInfo {
	constructor(public label: string, public width: number, public eventCode: KeyboardCodes) {
...
export default class ProceduralKeyboardMesh extends Mesh {
	...
	constructor() {
		super(getChamferedBoxGeometry(width, height, depth), materialLibrary.keyboardPlastic);
		...
		for (const row of keyInfo) {
			...
			for (const key of row) {
				...
				if (key.label !== "") {
					const keyMeshHole = new KeyButtonHoleMesh(key.width + 0.004, 0.012, KEY_SCALE + 0.004, 0.001);
					this.add(keyMeshHole);
					keyMeshHole.position.set(cursorX, height - 0.005, cursorY);
					const keyMesh = new KeyButtonMesh(key.width, 0.01, KEY_SCALE, key.label, 0.0015);
					if (key.eventCode !== undefined && !buttonsByEventCode.has(key.eventCode)) {
	...
	onKeyCodeEvent = (eventCode: KeyboardCodes, down: boolean) => {
		if (this.buttonsByEventCode.has(eventCode)) {
			this.buttonsByEventCode.get(eventCode)!.position.y = down ? height - 0.012 : height - 0.0075;
		}
	};
}
